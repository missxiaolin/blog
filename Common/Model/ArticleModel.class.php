...
//	自动完成
    protected $_auto=array(
        ...
        array('thumb','uploads',3,'callback'),
    );
	
//	移动文件到指定目录
	public function uploads(){
		if(!empty(I('post.oldthumb'))){
			if(empty(I('post.thumb'))){//如果没有上传新的文件就直接放回旧的图片
				return I('post.oldthumb');
			}else{//否则删除旧的图片上传新的
				$oldimg = I('post.oldthumb');
				$info = pathinfo($oldimg);
				$oldpicimg=str_replace("_list.{$info['extension']}", ".{$info['extension']}", $oldimg);
				unlink($oldimg);
				unlink($oldpicimg);
			}
		}
//		创建文件夹
	    is_dir('./Uploads/'.date("Y-m-d")) || mkdir('./Uploads/'.date("Y-m-d"),0755,TRUE);
	    $str = I('post.thumb');
//	            新地址
	    $address = str_replace("Tmps", "Uploads", $str);
//	             复制
	    copy(I('post.thumb'), $address);
//		实例化图像类(后面也进行封装传参大小和缩放类型)
		$image = new \Think\Image();
//		打开./Uploads/2016-09-13/下面的图片
		$image->open(I('post.thumb'));
//		获取图片信息
		$info = pathinfo($address);
//		新图片
		$thumb_pic=str_replace(".{$info['extension']}", "_list.{$info['extension']}", $address);
//		按原图比例缩
		$image->thumb(175, 120,\Think\Image::IMAGE_THUMB_FIXED)->save($thumb_pic);
//	    复制好删除
		unlink(I('post.thumb'));
		return $thumb_pic;
	}
...
//			修改图片默认的title和alt
			$data['content'] = preg_replace('/title=\"(?<=").*?(?=")\"/','title="小林博客"',$data['content']);
            $data['content'] = preg_replace('/alt=\"(?<=").*?(?=")\"/','alt="小林博客"',$data['content']);
		}
//		将路径转换为相对路径
...
//		转义
        ...
        if($aid=$this->add()){
            ...
            foreach($list as $k=>$v){
                $sitemap .= "    <url>\r\n"."        <loc>".U('Index/Index/article',array('aid'=>$v['aid']),'',true)."</loc>\r\n"."        <lastmod>".date('Y-m-d',$v['addtime'])."</lastmod>\r\n        <changefreq>weekly</changefreq>\r\n        <priority>0.8</priority>\r\n    </url>\r\n";
            }
            $sitemap .= '</urlset>';
    ...
    public function getPageData($cid='all',$tid='all',$is_show='1',$is_delete=0,$limit=10){
        if($cid=='all' && $tid=='all'){
            ...
            $count=$this
                ->where($where)
                ->count();
            $page=new \Common\Lib\Page($count,$limit);
            $list=$this
                ->where($where)
                ->order('addtime desc')
        ...
        foreach ($list as $k => $v) {
            ...
            $list[$k]['url']=U('Index/Index/article/',array('aid'=>$v['aid']));
    ...
    public function getDataByAid($aid,$map=''){
        if($map==''){
            ...
            // 如果不为空 添加url
            if(!empty($data['prev'])){
                $data['prev']['url']=U('Index/Index/content/',array('aid'=>$data['prev']['aid']));
            }
            if(!empty($data['next'])){
                $data['next']['url']=U('Index/Index/content/',array('aid'=>$data['next']['aid']));
            }
...
//      删除文章的同时还要删除图片
        $oldimg = $this->where(array('aid'=>$aid))->getField('thumb');
        $info = pathinfo($oldimg);
		$oldpicimg=str_replace("_list.{$info['extension']}", ".{$info['extension']}", $oldimg);
		unlink($oldimg);
		unlink($oldpicimg);
